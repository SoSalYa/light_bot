...
class DTEKChecker:
    ...
    async def init_browser_and_page(self):
        ...
        # Закрываем модальное окно с предупреждением (если есть)
        try:
            ...
            print("ℹ️ Модальное окно не найдено или уже закрыто")
        
        # Вводим данные адреса (опрос будем закрывать позже, при проверках)
